...
pro aurorax_example_atm_forward_custom_spectrum_and_neutral_profile
  ; --------------------------------------------------------------------------
  ; TREx-ATM Forward Calculation with both custom spectrum and neutral profile
  ; --------------------------------------------------------------------------
  ;
  ; We can also supply both a custom spectrum as well as a custom neutral profile. For
  ; more information about each, refer to their individual respective crib sheets.
  ;
  ; Complete documentation describing all caveats can be found in the
  ; `aurorax_atm_forward()` function's docs.
  
  ; First, let's create a custom spectrum
  ; 
  ef_count = 11  ; Number of energy and flux values we want
  custom_energy = []
  custom_flux = []
  for i=0, ef_count-1 do begin
    custom_energy = [custom_energy, 4000. + i * 100.]
    custom_flux = [custom_flux, 1e6]
  endfor
  ; Create custom spectrum hash
  custom_spectrum = {energy: custom_energy, flux: custom_flux}
  
  ; Next, let's create a custom neutral profile
  ; 
  ...
  ; run model, using our custom spectrum and neutral profile as inputs
  ...
  p_1356 = plot(alt, data.emission_1356, name='135.6 nm', color='hot pink', /overplot, xrange=[80,800], margin=[0.325,0.25,0.1,0.05], thick=3)
  p_blue = plot(alt, data.emission_4278, name='427.8 nm', color='dodger blue', layout=[1,6,2], /current, xrange=[80,800], margin=[0.325,0.25,0.1,0.05], thick=3)
  ...
  p_o_density = plot(alt, data.plasma_oplus_density, name='O$^{+}$', color='red', /overplot, xrange=[80,800], thick=3)
