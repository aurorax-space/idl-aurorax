...
function __aurorax_asi_pgm_readfile,filename,LUN=lun,VERBOSE=verbose,COMMENTS=comments,TUPLE_TYPE=tuple_type,MAXVAL=maxval
  ...
  if (width le 0) then begin & message,'[aurorax_read] Error - invalid width:' + string(width) & return,-1L & endif
  if (height le 0) then begin & message,'[aurorax_read] Error - invalid height:' + string(height) & return,-1L & endif
  if (depth le 0) then begin & message,'[aurorax_read] Error - invalid depth:' + string(depth) & return,-1L & endif
  if (maxval le 0) then begin & message,'[aurorax_read] Error - invalid maxval:' + string(maxval) & return,-1L & endif
  if (maxval gt 65535L) then begin & message,'[aurorax_read] Error - invalid maxval:' + string(maxval) & return,-1L & endif
