...
pro __aurorax_ucalgary_readfile_grid,grid_file_path,data,timestamp_list,meta,first_frame=first_frame

  ; Reading the grid data into an
  ; array object to be returned.
  file_id = h5f_open(grid_file_path)
  ...
  if size(grid, /n_dimensions) eq 3 then begin
    grid = transpose(grid, [1, 2, 0])
    confidence = transpose(confidence, [1, 2, 0])
  endif else if size(grid, /n_dimensions) eq 4 then begin
    grid = transpose(grid, [1, 2, 3, 0])
    confidence = transpose(confidence, [1, 2, 0])
  endif
  ...
  meta = {timestamp:timestamp, file_meta:file_meta, frame_meta:frame_meta}
  data = {grid:grid, confidence:confidence}
  h5_close
